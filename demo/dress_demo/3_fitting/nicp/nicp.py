...
def non_rigid_icp_mesh2pcl(
    ...
    w_edge_scale = config["edge_weights_scale"]
    w_normal = laplacian_weight * 0.1
    ...
    for i in loop:
        ...
        for _ in inner_loop:
            ...
            vert_distance_mask = torch.sum(vert_distance, dim = 2) < 1 #0.04**2
            ...
            w_edge = w_edge_scale[w_idx] * laplacian_weight
            edge_loss = mesh_edge_loss(new_deform_mesh) * w_edge
            # normal_loss = mesh_normal_consistency(new_deform_mesh) * w_normal
            
            loss = torch.sqrt(vert_sum + landmark_sum + stiffness_sum) + laplacian_loss + edge_loss
            loss.backward()
            optimizer.step()
            new_deformed_verts, stiffness = local_affine_model(transformed_vertex, pool_num = 0, return_stiff = True)
            new_deformed_lm = batch_vertex_sample(template_lm_index, new_deformed_verts)
