...
class SMPL_Layer(Module):
    ...
    def forward(self,
        ...
        ## self.th_J_regressor：torch.Size([24, 6890]), verts to keypoints
        ## self.th_shapedirs：torch.Size([6890, 3, 10]), shape offsets
        ...
        ## self.kintree_parents定义了除了根节点以外的每个节点的父节点
        ## root_rot.shape = torch.Size([1, 3, 3]), 根节点的rotation matrix
        ## th_pose_rotmat.shape = torch.Size([1, 207]), 除了根节点以外的rotation matrix
        ## th_j.shape = torch.Size([1, 24, 3]), 把shape加进来以后的joints坐标

        ## 把相对的平移和旋转转化成global的平移和旋转
        ...
        ## len(th_results_global) = 24
        ## th_results_global[0].shape = torch.Size([1, 24, 4, 4])
        ## 每一个元素都是一个变换矩阵，其中平移分量就是3d keypoints的坐标
        th_results_global = th_results

        ## th_results2.shape = torch.Size([1, 4, 4, 24])
        ...
        ## self.th_weights.shape = torch.Size([6890, 24])
        ## th_T.shape = torch.Size([1, 4, 4, 6890])
        th_T = torch.matmul(th_results2, self.th_weights.transpose(0, 1))

        ## th_rest_shape_h.shape = torch.Size([1, 4, 6890])
        ...
        ## th_verts.shape = torch.Size([1, 6890, 4])
