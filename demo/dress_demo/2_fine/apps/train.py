...
def train(opt):
    ...
    train_dataset = TrainDataset_Cari(opt, phase='train')
    test_dataset = TrainDataset_Cari(opt, phase='test')
    #for i in range(len(test_dataset)):
    #    print(test_dataset[i]['name'])
    #exit()
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        if epoch % opt.freq_save == 0 and train_idx != 0:
            ...
            with torch.no_grad():
                ...
                if not opt.no_gen_mesh:
                    ...
                    for gen_idx in tqdm(range(opt.num_gen_mesh_test)):
                        #idt = random.randint(0,len(test_dataset))
                        #test_data = random.choice(test_dataset)
                        idt = count * 8
                        test_data = test_dataset[idt]
                        if not os.path.exists('%s/%s/test_eval_epoch%d' % (opt.results_path, opt.name, epoch)):
                            os.mkdir('%s/%s/test_eval_epoch%d' % (opt.results_path, opt.name, epoch))
                        save_path = '%s/%s/test_eval_epoch%d/%s.obj' % (
                            opt.results_path, opt.name, epoch, test_data['name'].replace('/','_'))
                    ...
                    for gen_idx in tqdm(range(opt.num_gen_mesh_test)):
                        ...
                        #idt = random.randint(0,len(train_dataset))
                        if not os.path.exists('%s/%s/train_eval_epoch%d' % (opt.results_path, opt.name, epoch)):
                            os.mkdir('%s/%s/train_eval_epoch%d' % (opt.results_path, opt.name, epoch))                    
                        idt = count * 8
                        train_data = train_dataset[idt]
                        save_path = '%s/%s/train_eval_epoch%d/%s.obj' % (
                            opt.results_path, opt.name, epoch, train_data['name'].replace('/','_'))
